...
from dataset.utils import collect_tensor_result, grounding_eval_bbox, grounding_eval_bbox_vlue, pascalparts_grounding_eval_bbox
...
def val(model, data_loader, tokenizer, device):
    ...
    result = []
    for image, text, bbox in metric_logger.log_every(data_loader, print_freq, header):
        ...
        bbox = torch.stack(bbox).T
        assert len(bbox) == outputs_coord.shape[0]

        for box, coord in zip(bbox, outputs_coord):
            result.append({'bbox': box,'image_wh':image.shape[-2:], 'pred':coord,})
...
def main(args, config):
    ...
    grd_train_dataset, grd_test_dataset = create_dataset('pascalparts_grounding_bbox', config, args.evaluate)
    ...
    if args.evaluate:
        ...
        results = collect_tensor_result(result, filename='pascalparts_grounding_bbox_eval', local_wdir=args.result_dir,
        ...
        if utils.is_main_process():
            if 'vlue_test' in config.keys() and config['vlue_test']:
                ...
                grounding_acc = pascalparts_grounding_eval_bbox(results)
        ...
        for epoch in range(0, max_epoch):
            ...
            if utils.is_main_process():
                ...
                if grounding_acc['pascal_val'] > best:
                    save_obj = {
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', type=str, default='configs/Grounding_bbox_PascalParts.yaml')
    parser.add_argument('--output_dir', type=str, default='output/grounding_pascalparts')
